...
import { describeLiveValueFreshness } from './liveValueFreshness';
...
const copy: Record<RuntimeTagInspectorLocale, Copy> = {
  'pt-BR': {
    ...
    valueFreshness: 'Atualização do valor', freshnessFresh: 'Atualizada', freshnessAging: 'Dados envelhecendo', freshnessStale: 'Dados desatualizados', freshnessUnavailable: 'Sem timestamp de atualização'
  },
  en: {
    title: 'TAG Inspector',
    ...
    valueFreshness: 'Value freshness', freshnessFresh: 'Updated', freshnessAging: 'Data aging', freshnessStale: 'Data stale', freshnessUnavailable: 'No update timestamp'
  },
  es: {
    title: 'Inspector de TAGs',
    ...
    valueFreshness: 'Actualización del valor', freshnessFresh: 'Actualizado', freshnessAging: 'Datos envejeciendo', freshnessStale: 'Datos obsoletos', freshnessUnavailable: 'Sin timestamp de actualización'
  }
};
...
export function RuntimeTagInspector({
  ...
  const selectedFreshness = useMemo(
    () => describeLiveValueFreshness(selectedTag?.current?.timestamp, Date.now(), Math.max(refreshIntervalMs * 3, 15_000)),
    [refreshIntervalMs, selectedTag?.current?.timestamp, tags]
  );
...
function Fact({ label, value, mono = false, tone }: { label: string; value: string; mono?: boolean; tone?: ReturnType<typeof describeLiveValueFreshness>['state'] }) {
  return <div><dt>{label}</dt><dd className={`${mono ? 'mono ' : ''}${tone ? `runtime-tag-freshness freshness-${tone}` : ''}`.trim()}>{value}</dd></div>;
}

function freshnessText(state: ReturnType<typeof describeLiveValueFreshness>['state'], text: Copy) {
