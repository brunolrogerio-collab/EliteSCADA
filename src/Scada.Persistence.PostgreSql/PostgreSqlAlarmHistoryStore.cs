...
public sealed class PostgreSqlAlarmHistoryStore : IHistoricalDatasetProvider, IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(query.Search))
        {
            ...
            command.Parameters.AddWithValue(name, NpgsqlDbType.Text, $"%{EscapeLike(query.Search)}%");
            sql.Append($" AND (tag_path ILIKE @{name} ESCAPE '\\' OR COALESCE(message, '') ILIKE @{name} ESCAPE '\\')");
        }
        ...
        {
            if (!Guid.TryParse(query.After.TieBreaker, out var eventId))
            ...
            {
                AddAfterPrimary(query.Sort.Field, query.After.Primary, command);
        ...
        ref int parameterIndex)
    {
        ...
        var parameter = $"p{parameterIndex++}";
        var value = filter.Values[0].Value!;
        if (filter.Operator is HistoricalFilterOperator.Contains or HistoricalFilterOperator.StartsWith)
        {
            value = filter.Operator == HistoricalFilterOperator.Contains
                ? $"%{EscapeLike(value)}%"
                : $"{EscapeLike(value)}%";
        }
        command.Parameters.AddWithValue(parameter, NpgsqlDbType.Text, value);
        return filter.Operator switch
        {
            HistoricalFilterOperator.Eq => $"{column} = @{parameter}",
            HistoricalFilterOperator.NotEq => $"{column} <> @{parameter}",
            HistoricalFilterOperator.Contains or HistoricalFilterOperator.StartsWith => $"{column} ILIKE @{parameter} ESCAPE '\\'",
        ...
        ref int parameterIndex)
    {
        var values = filter.Values.Select(value =>
        {
            ...
            if (numeric != Math.Truncate(numeric) || numeric < (int)AlarmPriority.Low || numeric > (int)AlarmPriority.Critical)
        ...
        NpgsqlCommand command)
    {
        switch (field)
        {
            ...
            case "state":
                ...
                break;
            case "tag.path":
                command.Parameters.AddWithValue("after_primary", NpgsqlDbType.Text, value.Value ?? throw new HistoricalQueryCursorException("Alarm TAG path cursor value is invalid."));
                break;
            default:
    ...
    private static HistoricalQueryRow ToRow(AlarmHistoryRow row) =>
        new(new Dictionary<string, HistoricalQueryValue>(StringComparer.Ordinal)
        {
            ...
            ["message"] = row.Message is null ? HistoricalQueryValue.Null() : HistoricalQueryValue.FromString(row.Message),
