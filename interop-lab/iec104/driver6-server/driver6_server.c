...
static bool interrogationHandler(void* parameter, IMasterConnection connection, CS101_ASDU request, uint8_t qoi)
{
    ...
    CS101_ASDU data = CS101_ASDU_create(
        alParams,
        false,
        CS101_COT_INTERROGATED_BY_STATION,
        (uint8_t)CS101_ASDU_getOA(request),
        1,
        false,
        false);

    InformationObject scaled = (InformationObject)MeasuredValueScaled_create(
        NULL,
        100,
        23,
...
int main(void)
{
    ...
    printf("Driver 6 IEC-104 L2 peer listening on 0.0.0.0:2404\n");
    fflush(stdout);
    ...
    while (g_running)
    {
        ...
        if (g_activated && now >= lastSpontaneous + 500)
        {
            ...
            CS101_ASDU spontaneous = CS101_ASDU_create(
                alParams,
                false,
                CS101_COT_SPONTANEOUS,
                0,
