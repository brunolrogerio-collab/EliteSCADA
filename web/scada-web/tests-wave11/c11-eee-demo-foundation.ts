...
function buildServerScriptSource(): string {
  ...
  return [
    'def on_initialize():',
    `    write_server_memory("${t.cmdAutoEnable}", False)`, `    write_server_memory("${t.cmdAutoDisable}", False)`,
    `    write_server_memory("${t.cmdP01Start}", False)`, `    write_server_memory("${t.cmdP01Stop}", False)`,
    `    write_server_memory("${t.cmdP02Start}", False)`, `    write_server_memory("${t.cmdP02Stop}", False)`,
    `    write_server_memory("${t.cmdResetFaults}", False)`, `    write_server_memory("${t.cmdInjectP01Fault}", False)`,
    `    write_server_memory("${t.cmdInjectP02Fault}", False)`, `    write_server_memory("${t.cmdHighDemandEnable}", False)`,
    `    write_server_memory("${t.cmdHighDemandDisable}", False)`, `    write_server_memory("${t.cmdBadQualityEnable}", False)`,
    `    write_server_memory("${t.cmdBadQualityDisable}", False)`, '',
    'def on_tick():',
    `    level = read_server_memory("${t.levelPct}")`, `    auto_mode = read_server_memory("${t.autoMode}")`,
    `    high_demand = read_server_memory("${t.highDemand}")`, `    bad_quality = read_server_memory("${t.badQualityScenario}")`,
    `    duty = read_server_memory("${t.dutyPump}")`, `    cycles = read_server_memory("${t.cycleCount}")`,
    `    p01_running = read_server_memory("${t.p01Running}")`, `    p02_running = read_server_memory("${t.p02Running}")`,
    `    p01_available = read_server_memory("${t.p01Available}")`, `    p02_available = read_server_memory("${t.p02Available}")`,
    `    p01_fault = read_server_memory("${t.p01Fault}")`, `    p02_fault = read_server_memory("${t.p02Fault}")`,
    `    p01_trip = read_server_memory("${t.p01Trip}")`, `    p02_trip = read_server_memory("${t.p02Trip}")`,
    '    old_auto_mode = auto_mode', '    old_high_demand = high_demand', '    old_bad_quality = bad_quality', '    old_duty = duty',
    '    old_p01_running = p01_running', '    old_p02_running = p02_running', '    old_p01_fault = p01_fault', '    old_p02_fault = p02_fault',
    `    if read_server_memory("${t.cmdAutoEnable}"):`,'        auto_mode = True',
    `    if read_server_memory("${t.cmdAutoDisable}"):`,'        auto_mode = False',
    `    if read_server_memory("${t.cmdHighDemandEnable}"):`,'        high_demand = True',
    `    if read_server_memory("${t.cmdHighDemandDisable}"):`,'        high_demand = False',
    `    if read_server_memory("${t.cmdBadQualityEnable}"):`,'        bad_quality = True',
    `    if read_server_memory("${t.cmdBadQualityDisable}"):`,'        bad_quality = False',
    `    if read_server_memory("${t.cmdInjectP01Fault}"):`,'        p01_fault = True','        p01_trip = True',
    `    if read_server_memory("${t.cmdInjectP02Fault}"):`,'        p02_fault = True','        p02_trip = True',
    `    if read_server_memory("${t.cmdResetFaults}"):`,'        p01_fault = False','        p02_fault = False','        p01_trip = False','        p02_trip = False',
    '    if auto_mode:',
    ...
    '        if level <= 35.0 and (p01_running or p02_running):','            p01_running = False','            p02_running = False','            cycles = cycles + 1',
    '            if duty == 1:','                duty = 2','            else:','                duty = 1',
    '    else:',
    `        if read_server_memory("${t.cmdP01Start}") and p01_available and not p01_fault and not p01_trip:`,'            p01_running = True',
    `        if read_server_memory("${t.cmdP01Stop}"):`,'            p01_running = False',
    `        if read_server_memory("${t.cmdP02Start}") and p02_available and not p02_fault and not p02_trip:`,'            p02_running = True',
    `        if read_server_memory("${t.cmdP02Stop}"):`,'            p02_running = False',
    ...
    '    p01_flow = 0.0','    p02_flow = 0.0','    p01_frequency = 0.0','    p02_frequency = 0.0','    p01_current = 0.0','    p02_current = 0.0','    station_pressure = 0.0',
    '    if p01_running and p02_running:','        p01_flow = 35.0','        p02_flow = 35.0','        p01_frequency = 45.0','        p02_frequency = 45.0','        p01_current = 20.0','        p02_current = 20.0','        station_pressure = 3.0',
    '    else:',
    ...
    `    write_server_memory("${t.levelPct}", level)`, `    write_server_memory("${t.inflowM3h}", inflow)`,
    `    write_server_memory("${t.totalFlowM3h}", total_flow)`, `    write_server_memory("${t.dischargePressureBar}", station_pressure)`,
    `    write_server_memory("${t.autoMode}", auto_mode)`, `    write_server_memory("${t.highDemand}", high_demand)`,
    `    write_server_memory("${t.badQualityScenario}", bad_quality)`, `    write_server_memory("${t.dutyPump}", duty)`, `    write_server_memory("${t.cycleCount}", cycles)`,
    `    write_server_memory("${t.p01Running}", p01_running)`, `    write_server_memory("${t.p01Fault}", p01_fault)`, `    write_server_memory("${t.p01Trip}", p01_trip)`,
    `    write_server_memory("${t.p01FrequencyHz}", p01_frequency)`, `    write_server_memory("${t.p01CurrentA}", p01_current)`, `    write_server_memory("${t.p01FlowM3h}", p01_flow)`,
    `    write_server_memory("${t.p02Running}", p02_running)`, `    write_server_memory("${t.p02Fault}", p02_fault)`, `    write_server_memory("${t.p02Trip}", p02_trip)`,
    `    write_server_memory("${t.p02FrequencyHz}", p02_frequency)`, `    write_server_memory("${t.p02CurrentA}", p02_current)`, `    write_server_memory("${t.p02FlowM3h}", p02_flow)`,
    `    write_server_memory("${t.p02PressureBar}", station_pressure if p02_running else 0.0)`,
    '    if bad_quality:', `        publish_server_memory_sample("${t.p01PressureBar}", station_pressure if p01_running else 0.0, "Unavailable")`,
    '    else:', `        write_server_memory("${t.p01PressureBar}", station_pressure if p01_running else 0.0)`,
