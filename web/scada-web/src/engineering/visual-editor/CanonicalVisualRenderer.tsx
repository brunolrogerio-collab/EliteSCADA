  ...
  try {
    ...
    if (element.type === BUILTIN_VISUAL_OBJECT_TYPES.image) {
      ...
      return <div className="visual-editor-object visual-editor-image" style={style} data-object-id={element.id ?? undefined}>
        {assetId ? <img
          src={visualAssetContentUrl(assetId)}
          alt={element.key}
          draggable={false}
          style={{
            width: '100%', height: '100%', objectFit: imageFit(values[VISUAL_PROPERTY_KEYS.imageFit]),
            objectPosition: `${percent(values[VISUAL_PROPERTY_KEYS.imagePositionX])}% ${percent(values[VISUAL_PROPERTY_KEYS.imagePositionY])}%`
          }}
...
function elementStyle(values: Readonly<Record<string, VisualPropertyValue>>): CSSProperties {
  ...
  return {
    ...
    fontSize: numberValue(values[VISUAL_PROPERTY_KEYS.fontSize], 14),
    fontWeight: numberValue(values[VISUAL_PROPERTY_KEYS.fontWeight], 400),
    ...
    alignItems: verticalAlignment(values[VISUAL_PROPERTY_KEYS.verticalAlignment]),
    justifyContent: horizontalFlexAlignment(values[VISUAL_PROPERTY_KEYS.horizontalAlignment])
  };
}

function lineStyle(base: CSSProperties, values: Readonly<Record<string, VisualPropertyValue>>): CSSProperties {
  return {
    ...base, height: 0, minHeight: 0, overflow: 'visible', background: 'transparent', borderWidth: 0,
    borderTopWidth: numberValue(values[VISUAL_PROPERTY_KEYS.strokeWidth], 1),
    borderTopColor: stringValue(values[VISUAL_PROPERTY_KEYS.strokeColor], '#000000'),
    borderTopStyle: stringValue(values[VISUAL_PROPERTY_KEYS.strokeStyle], 'solid') as CSSProperties['borderTopStyle']
  };
}

function assetReferenceId(value: VisualPropertyValue | undefined): string | null {
  if (!value || typeof value !== 'object' || !('assetId' in value)) return null;
  return typeof value.assetId === 'string' && value.assetId.length > 0 ? value.assetId : null;
}
function imageFit(value: VisualPropertyValue | undefined): CSSProperties['objectFit'] {
  const fit = stringValue(value, 'contain');
  return fit === 'cover' ? 'cover' : fit === 'fill' ? 'fill' : fit === 'native' ? 'none' : 'contain';
}
...
function verticalAlignment(value: VisualPropertyValue | undefined): CSSProperties['alignItems'] {
  ...
  return alignment === 'top' ? 'flex-start' : alignment === 'bottom' ? 'flex-end' : 'center';
}
function horizontalFlexAlignment(value: VisualPropertyValue | undefined): CSSProperties['justifyContent'] {
  const alignment = stringValue(value, 'left');
