...
export function isVisualElementEffectivelyAuthoringLocked(
  ...
  let current = requireElement(screen, objectId);
  ...
  while (parent) {
    ...
    if (!parent.id) break;
    current = parent;
    parent = findParent(screen.elements ?? [], current.id);
  }
  return false;
}
...
function alignElements(
  ...
  selected.forEach((element, index) => {
    ...
    switch (operation) {
      ...
      case 'horizontalCenter': x = horizontalCenter - current.width / 2; break;
      case 'right': x = right - current.width; break;
      case 'top': y = top; break;
      case 'verticalMiddle': y = verticalMiddle - current.height / 2; break;
      case 'bottom': y = bottom - current.height; break;
    }
