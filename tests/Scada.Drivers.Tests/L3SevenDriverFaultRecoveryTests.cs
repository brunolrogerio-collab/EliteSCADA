...
public sealed class L3SevenDriverFaultRecoveryTests
{
    private const string OpcUaSecurityPolicyNone = "http://opcfoundation.org/UA/SecurityPolicy#None";

    [Fact]
    [Trait("Category", "L3SevenDriverFaultRecovery")]
    public async Task OneRuntime_SurvivesSerialPeerLoss_AndRecoversEachDriverWithoutHostRestart()
    {
        if (!FaultInjectionEnabled() || !TryGetLab(out var lab)) return;
        ...
        var package = new EngineeringPackage(
            ...
            [
                Tag(mqttId, "MQTT", "L3.Fault.MQTT", TagDataType.Double, "l3.fault.mqtt", mqttBinding, true),
                Tag(iec104Id, "IEC104", "L3.Fault.IEC104", TagDataType.Int16, "l3.fault.iec104", iecBinding, true),
                Tag(cipId, "CIP", "L3.Fault.CIP", TagDataType.Int16, "l3.fault.cip", cipBinding, false),
                Tag(opcUaId, "OPCUA", "L3.Fault.OPCUA", TagDataType.Double, "l3.fault.opcua", opcBinding, false),
                Tag(dnp3Id, "DNP3", "L3.Fault.DNP3", TagDataType.Int32, "l3.fault.dnp3", dnpBinding, true),
                Tag(s7Id, "S7", "L3.Fault.S7", TagDataType.Int16, "l3.fault.s7", s7Binding, false),
                Tag(bacnetId, "BACnet", "L3.Fault.BACnet", TagDataType.Double, "l3.fault.bacnet", bacnetBinding, false)
            ],
        ...
        var activation = await runtime.ActivateAsync("l3-seven-driver-fault-recovery", 1, package);
        Assert.True(activation.Activated, Describe(activation));
        Assert.Equal(7, runtime.Describe().CommunicationDrivers.Count);

        await WaitForGoodValueAsync(runtime, iec104Id, value => Convert.ToInt16(value) == 23, TimeSpan.FromSeconds(15));
        await WaitForGoodValueAsync(runtime, cipId, value => Convert.ToInt16(value) == 1234, TimeSpan.FromSeconds(15));
        await WaitForGoodValueAsync(runtime, opcUaId, value => Math.Abs(Convert.ToDouble(value) - 21.5d) < 0.001d, TimeSpan.FromSeconds(15));
        await WaitForGoodValueAsync(runtime, dnp3Id, value => Convert.ToInt32(value) == 4242, TimeSpan.FromSeconds(15));
        await WaitForGoodValueAsync(runtime, s7Id, value => Convert.ToInt16(value) == 1234, TimeSpan.FromSeconds(15));
        await WaitForGoodValueAsync(runtime, bacnetId, value => Math.Abs(Convert.ToDouble(value) - 21.5d) < 0.001d, TimeSpan.FromSeconds(15));
        await PublishMqttEventuallyAsync(lab.NodeRedUrl, 40.0d, TimeSpan.FromSeconds(10));
        await WaitForGoodValueAsync(runtime, mqttId, value => Math.Abs(Convert.ToDouble(value) - 40d) < 0.001d, TimeSpan.FromSeconds(10));

        short cipCanary = 4300;
        async Task ProbeCipAsync()
        {
            cipCanary++;
            await runtime.WriteAsync(cipId, cipCanary);
            await WaitForCipValueAsync(lab.CipHost, lab.CipPort, cipReference, cipCanary, TimeSpan.FromSeconds(10));
        }
        ...
        await ExercisePeerFaultAsync(
            runtime,
            "elitescada-lab-iec104-lib60870",
            ProbeCipAsync,
            () => WaitForGoodValueAsync(runtime, iec104Id, value => Convert.ToInt16(value) == 23, TimeSpan.FromSeconds(20)));

        await ExercisePeerFaultAsync(
            runtime,
            "elitescada-lab-cip-controllogix",
            async () =>
            {
                await runtime.WriteAsync(opcUaId, 42.25d);
                await WaitForGoodValueAsync(runtime, opcUaId, value => Math.Abs(Convert.ToDouble(value) - 42.25d) < 0.001d, TimeSpan.FromSeconds(10));
            },
            () => WaitForGoodValueAsync(runtime, cipId, value => Convert.ToInt16(value) == 1234, TimeSpan.FromSeconds(20)));

        await ExercisePeerFaultAsync(
            runtime,
            "elitescada-lab-opcua-open62541",
            ProbeCipAsync,
            () => WaitForGoodValueAsync(runtime, opcUaId, value => Math.Abs(Convert.ToDouble(value) - 21.5d) < 0.001d, TimeSpan.FromSeconds(20)));

        await ExercisePeerFaultAsync(
            runtime,
            "elitescada-lab-dnp3-dnp3py",
            ProbeCipAsync,
            () => WaitForGoodValueAsync(runtime, dnp3Id, value => Convert.ToInt32(value) == 4242, TimeSpan.FromSeconds(20)));

        await ExercisePeerFaultAsync(
            runtime,
            "elitescada-lab-s7-python-snap7",
            ProbeCipAsync,
            () => WaitForGoodValueAsync(runtime, s7Id, value => Convert.ToInt16(value) == 1234, TimeSpan.FromSeconds(20)));

        await ExercisePeerFaultAsync(
            runtime,
            "elitescada-lab-bacnet-bacpypes",
            ProbeCipAsync,
            () => WaitForGoodValueAsync(runtime, bacnetId, value => Math.Abs(Convert.ToDouble(value) - 21.5d) < 0.001d, TimeSpan.FromSeconds(25)));
    }

    private static async Task ExercisePeerFaultAsync(
        GatewayEngineeringRuntimeCoordinator runtime,
        string containerName,
        Func<Task> unaffectedProbe,
        Func<Task> recoveryProbe)
    {
        await DockerAsync("stop", "-t", "2", containerName);
        try
        {
            await Task.Delay(750);
            await unaffectedProbe();
            Assert.Equal(7, runtime.Describe().CommunicationDrivers.Count);
        }
        finally
        {
            await DockerAsync("start", containerName);
        }
    ...
    private static DataSourceEngineeringDto MqttDataSource(string host, int port) => new(
        Guid.NewGuid(), "l3.fault.mqtt", "L3 Fault MQTT", MqttDriverDescriptorProvider.DriverType,
        Settings: new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["host"] = host,
            ["port"] = port.ToString(System.Globalization.CultureInfo.InvariantCulture),
            ["tls"] = "false",
            ["clientId"] = $"elite-l3-fault-{Guid.NewGuid():N}",
            ["protocolVersion"] = "mqtt5"
        });

    private static DataSourceEngineeringDto Iec104DataSource(string host, int port) => new(
        Guid.NewGuid(), "l3.fault.iec104", "L3 Fault IEC-104", Iec104EngineeringConnectionTester.DriverType,
        Settings: new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["host"] = host,
            ["port"] = port.ToString(System.Globalization.CultureInfo.InvariantCulture),
    ...
    private static DataSourceEngineeringDto CipDataSource(string host, int port) => new(
        Guid.NewGuid(), "l3.fault.cip", "L3 Fault CIP", AllenBradleyLogixContractIdentity.DriverType,
        Settings: new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["host"] = host,
            ["port"] = port.ToString(System.Globalization.CultureInfo.InvariantCulture),
    ...
    private static DataSourceEngineeringDto OpcUaDataSource(string endpoint) => new(
        Guid.NewGuid(), "l3.fault.opcua", "L3 Fault OPC UA", OpcUaDriverDescriptorProvider.DriverTypeId,
        Settings: new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
    ...
    private static DataSourceEngineeringDto Dnp3DataSource(string host, int port) => new(
        Guid.NewGuid(), "l3.fault.dnp3", "L3 Fault DNP3", Dnp3DriverDescriptorProvider.DriverType,
        Settings: new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["transport"] = "tcp",
            ["host"] = host,
    ...
    private static DataSourceEngineeringDto S7DataSource(string host, int port) => new(
        Guid.NewGuid(), "l3.fault.s7", "L3 Fault S7", S7IsoCommunicationRuntimePlan.DriverTypeKey,
        Settings: new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["host"] = host,
            ["port"] = port.ToString(System.Globalization.CultureInfo.InvariantCulture),
    ...
    private static DataSourceEngineeringDto BacnetDataSource() => new(
        Guid.NewGuid(), "l3.fault.bacnet", "L3 Fault BACnet/IP", BacnetDriverDescriptor.DriverType,
        Settings: new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
    ...
    private static async Task PublishMqttEventuallyAsync(string nodeRedUrl, double value, TimeSpan timeout)
    {
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            try
            {
                ...
                lastError = new InvalidOperationException($"Node-RED returned {(int)response.StatusCode} {response.ReasonPhrase}.");
            }
            catch (Exception ex)
            {
                lastError = ex;
        ...
        TimeSpan timeout)
    {
        ...
        throw new TimeoutException($"TAG '{tagId}' did not recover to expected Good value within {timeout}. Last={last?.Value}; Quality={last?.Quality}.");
    }
