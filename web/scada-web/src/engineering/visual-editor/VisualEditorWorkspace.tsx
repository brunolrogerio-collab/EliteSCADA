...
import { VISUAL_PROPERTY_KEYS } from '../../visual-runtime';
...
import {
  ...
  buildVisualEditorTagSourceCatalog,
...
export function VisualEditorWorkspace({ snapshot, locale, onApplied }: VisualEditorWorkspaceProps) {
  ...
  const tagSourceCatalog = useMemo(
    () => buildVisualEditorTagSourceCatalog(snapshot.package.tags ?? []),
  ...
  const canChooseImageAsset = selectedElement?.type === 'core.image' && Boolean(selectedElement.id);
        ...
        <div className="visual-editor-composition">
          <aside className="visual-editor-slot visual-editor-palette-slot">
            <ObjectPalette
              ...
              copy={{
                title: text.objectsPanel,
                hint: text.objectsPanelHint,
                addLabel: text.addObject,
                labels: text.objectLabels
              }}
            />
          ...
          <section className="visual-editor-canvas-slot">
            <header><div><strong>{draft.name || draft.key || text.untitled}</strong><code>{draft.route || text.noRoute}</code></div><span>{text.interactiveCanvas}</span></header>
            <VisualEditorCanvas
              screen={draft}
          ...
          <aside className="visual-editor-slot visual-editor-inspector-slot">
            ...
            {selectedElement ? (
              <BindingEditor
                ...
                sourceCatalog={tagSourceCatalog}
                onMutationIntent={handleMutationIntent}
                copy={{
                  title: text.binding,
...
function readAssetId(element: VisualElementEngineering): string {
  ...
  return value !== null && typeof value === 'object' ? value.assetId : '';
}
...
function visualEditorText(locale: EngineeringLocale) {
  if (locale === 'en') return {
    ...
    name: 'Name', key: 'Key', route: 'Route', draft: 'Draft', newDraft: 'New', changed: 'Changed', unchanged: 'Unchanged', untitled: 'Untitled Screen', objectsPanel: 'Objects', objectsPanelHint: 'Add registered visual objects. Identity and defaults remain coordinator/canonical authority.', addObject: 'Add', objectLabels: { group: 'Group', rectangle: 'Rectangle', ellipse: 'Ellipse', line: 'Line', text: 'Text', image: 'Image', valueDisplay: 'Value display', button: 'Button' },
    propertiesPanel: 'Properties', canonicalPreview: 'Canonical rendered preview', canonicalPreviewHint: 'Renderer projection of the same Engineering draft.', interactiveCanvas: 'Interactive Canvas', emptyCanvas: 'This Screen has no canonical visual objects yet.', reset: 'Reset draft', preview: 'Preview change', previewing: 'Previewing...', apply: 'Apply to Workspace', applying: 'Applying...',
    ...
    binding: 'Binding', bindingDestination: 'Visual property', bindingSource: 'TAG source', applyBinding: 'Apply binding', removeBinding: 'Remove binding', noBindingDestinations: 'This object has no bindable visual properties.', noBindingSources: 'No canonical TAG sources are available.', currentBinding: 'Current binding', selectBindingObject: 'Select one visual object to edit its canonical binding.',
  ...
  if (locale === 'es') return {
    ...
    name: 'Nombre', key: 'Clave', route: 'Ruta', draft: 'Borrador', newDraft: 'Nuevo', changed: 'Modificado', unchanged: 'Sin cambios', untitled: 'Pantalla sin título', objectsPanel: 'Objetos', objectsPanelHint: 'Agregue objetos visuales registrados. La identidad y los valores predeterminados siguen bajo autoridad canónica.', addObject: 'Agregar', objectLabels: { group: 'Grupo', rectangle: 'Rectángulo', ellipse: 'Elipse', line: 'Línea', text: 'Texto', image: 'Imagen', valueDisplay: 'Valor', button: 'Botón' },
    propertiesPanel: 'Propiedades', canonicalPreview: 'Preview renderizado canónico', canonicalPreviewHint: 'Proyección del renderer sobre el mismo borrador de Engineering.', interactiveCanvas: 'Canvas interactivo', emptyCanvas: 'Esta Pantalla todavía no contiene objetos visuales canónicos.', reset: 'Restablecer borrador', preview: 'Preview del cambio', previewing: 'Validando...', apply: 'Aplicar al Workspace', applying: 'Aplicando...',
    ...
    binding: 'Binding', bindingDestination: 'Propiedad visual', bindingSource: 'TAG de origen', applyBinding: 'Aplicar binding', removeBinding: 'Eliminar binding', noBindingDestinations: 'Este objeto no tiene propiedades visuales enlazables.', noBindingSources: 'No hay TAGs canónicos disponibles.', currentBinding: 'Binding actual', selectBindingObject: 'Seleccione un objeto visual para editar su binding canónico.',
  ...
  return {
    ...
    binding: 'Binding', bindingDestination: 'Propriedade visual', bindingSource: 'TAG de origem', applyBinding: 'Aplicar binding', removeBinding: 'Remover binding', noBindingDestinations: 'Este objeto não possui propriedades visuais com binding.', noBindingSources: 'Não há TAGs canônicos disponíveis.', currentBinding: 'Binding atual', selectBindingObject: 'Selecione um objeto visual para editar seu binding canônico.',
