...
public sealed class StepFunctionDnp3DriverWireIntegrationTests
{
    [Fact]
    public async Task Driver_PublishesRealWireMeasurementsAndExecutesCrobThroughCachePipeline()
    {
        ...
        try
        {
            ...
            await WaitUntilAsync(
                () => HasGoodBoolean(cache, binaryInputTag.Id, false) && HasGoodBoolean(cache, binaryOutputTag.Id, false),
                TimeSpan.FromSeconds(10),
                "Startup integrity did not populate the configured TAGs.");
            ...
            await WaitUntilAsync(
                () => HasGoodBoolean(cache, binaryInputTag.Id, true),
                TimeSpan.FromSeconds(10),
                "Class 1 Binary Input event did not reach CurrentTagCache.");

            Assert.True(cache.TryGet(binaryInputTag.Id, out var binaryInputValue));
            Assert.Equal(eventTime.ToUnixTimeMilliseconds(), binaryInputValue?.SourceTimestamp?.ToUnixTimeMilliseconds());
            Assert.Equal("dnp3-wire-driver", binaryInputValue?.Source);

            Assert.True(cache.TryGet(binaryOutputTag.Id, out var outputBeforeCommand));
            Assert.Equal(false, outputBeforeCommand?.Value);

            await driver.WriteAsync(binaryOutputTag.Id, true);

            Assert.True(controlHandler.OperateCount >= 1);
            await WaitUntilAsync(
                () => HasGoodBoolean(cache, binaryOutputTag.Id, true),
                TimeSpan.FromSeconds(10),
                "Binary Output Status feedback did not return through DNP3 into CurrentTagCache.");
    ...
    private static bool HasGoodBoolean(CurrentTagCache cache, Guid tagId, bool expected)
    {
        if (!cache.TryGet(tagId, out var value) || value is null) return false;
        return value.Quality == TagQuality.Good && value.Value is bool boolean && boolean == expected;
    }
    ...
    private static void InitializeDatabase(Step.Outstation outstation)
    {
        outstation.Transaction(db =>
        {
            ...
            db.AddBinaryOutputStatus(
                1,
                Step.EventClass.Class1,
                new Step.BinaryOutputStatusConfig());
            ...
            db.UpdateBinaryInput(new Step.BinaryInput(0, false, flags, timestamp), Step.UpdateOptions.NoEvent());
            db.UpdateBinaryOutputStatus(new Step.BinaryOutputStatus(1, false, flags, timestamp), Step.UpdateOptions.NoEvent());
        });
    }
