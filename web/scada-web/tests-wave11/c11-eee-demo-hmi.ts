import { buildEeeFoundationPackage, EEE_IDS, EEE_PATHS } from './c11-eee-demo-foundation-canonical';
...
export const EEE_HMI = {
  ...
  screens: {
    overview: { id: vid(7, 1), key: 'eee.overview' },
...
function rect(
  ...
  height: number,
  fillColor = C.panel,
  options: { borderColor?: string; borderWidth?: number; radius?: number; z?: number } = {}
) {
  const strokeWidth = options.borderWidth ?? 1;
  return {
    id,
    key,
    type: 'core.rectangle',
    properties: {
      ...
      fillStyle: 'solid',
      fillColor,
      strokeColor: options.borderColor ?? C.border,
      strokeWidth,
      strokeStyle: strokeWidth === 0 ? 'none' : 'solid',
      cornerRadius: options.radius ?? 12,
      zIndex: options.z ?? 1,
      visible: true
    }
  };
...
function ellipse(
  ...
  return {
    ...
    properties: {
      ...
      width,
      height,
      fillStyle: fillColor === '#00000000' ? 'none' : 'solid',
      fillColor,
      strokeColor,
      strokeWidth,
      strokeStyle: strokeWidth === 0 ? 'none' : 'solid',
...
function valueDisplay(
  ...
  return {
    ...
    bindings: [tagBinding(spec, parameter, decimalPlaces)]
  };
}
...
function line(id: string, key: string, x: number, y: number, width: number, strokeColor: string, strokeWidth: number, zIndex: number) {
  return {
    id,
    key,
    type: 'core.line',
...
const SCREEN_ORDER = [
  EEE_HMI.screens.overview.key,
  EEE_HMI.screens.instrumentation.key,
  EEE_HMI.screens.electrical.key,
  EEE_HMI.screens.operation.key,
  EEE_HMI.screens.trends.key,
  EEE_HMI.screens.alarmsEvents.key
] as const;

function navElements(activeKey: string) {
  const items = [
    [EEE_HMI.screens.overview.key, 'EEE PRINCIPAL'],
    [EEE_HMI.screens.instrumentation.key, 'INSTRUMENTAÇÃO'],
    [EEE_HMI.screens.electrical.key, 'ELÉTRICO'],
    [EEE_HMI.screens.operation.key, 'OPERAÇÃO'],
    [EEE_HMI.screens.trends.key, 'TENDÊNCIAS'],
  ...
  return [
    ...
    ...items.map(([key, label], index) => button(
      vid(9, 2200 + navHash(activeKey) * 10 + index),
      `nav-${activeKey}-${key}`,
      label,
      650 + index * 200,
      17,
...
function navHash(key: string) {
  return SCREEN_ORDER.indexOf(key as typeof SCREEN_ORDER[number]) + 1;
}

function pumpDynamo() {
  const parameters = ['running', 'fault', 'trip', 'current', 'frequency', 'flow', 'pressure']
    .map(key => ({ key, kind: 'tagReference', required: true, defaultValue: null, defaultTagReference: null, version: 1 }));
  const defaults = {
    running: T.p01Running,
    fault: T.p01Fault,
    trip: T.p01Trip,
    current: T.p01Current,
    frequency: T.p01Frequency,
    flow: T.p01Flow,
    pressure: T.p01Pressure
  };
  ...
  return {
    ...
    parameters,
    elements: [
      rect(vid(9, 3001), 'pump-card', 0, 0, 360, 430, C.panel, { borderColor: '#475569', borderWidth: 2, radius: 18, z: 1 }),
      text(vid(9, 3002), 'pump-title', 'CONJUNTO MOTOBOMBA', 28, 20, 304, 38, { fontSize: 21, weight: 800, align: 'center', z: 4 }),
      ellipse(vid(9, 3003), 'pump-body', 90, 72, 180, 144, C.off, '#CBD5E1', 4, 4, true),
      ...
      valueDisplay(vid(9, 3013), 'pump-frequency', 'Frequência da bomba', defaults.frequency, 196, 266, 145, 1, 'frequency'),
      text(vid(9, 3014), 'pump-flow-label', 'VAZÃO', 18, 334, 128, 24, { fontSize: 13, color: C.muted, weight: 700, z: 10 }),
      valueDisplay(vid(9, 3015), 'pump-flow', 'Vazão da bomba', defaults.flow, 18, 362, 145, 1, 'flow'),
      text(vid(9, 3016), 'pump-pressure-label', 'PRESSÃO', 196, 334, 146, 24, { fontSize: 13, color: C.muted, weight: 700, z: 10 }),
      valueDisplay(vid(9, 3017), 'pump-pressure', 'Pressão da bomba', defaults.pressure, 196, 362, 145, 2, 'pressure')
    ]
  };
}
...
function pumpInstance(
  ...
  y: number,
  popupKey: string,
  specs: { running: TagSpec; fault: TagSpec; trip: TagSpec; current: TagSpec; frequency: TagSpec; flow: TagSpec; pressure: TagSpec }
) {
  return {
    id,
    key,
...
function mainScreen() {
  const elements: any[] = [
    ...
    ...navElements(EEE_HMI.screens.overview.key),
    ...
    text(vid(9, 4011), 'well-title', 'POÇO DE SUCÇÃO', 84, 252, 480, 36, { fontSize: 22, weight: 800, align: 'center', z: 5 }),
    {
      ...
      properties: {
        ...
        zIndex: 5,
        visible: true,
        tooltip: 'Nível do poço de sucção'
      },
      analogFill: {
        ...
        source: {
          ...
          valueType: 'Number',
          target: T.level.path,
          tagReference: { tagId: T.level.id, selector: null },
          version: 1
        },
        inputMinimum: 0,
        inputMaximum: 100,
    ...
    rect(vid(9, 4030), 'discharge-header', 650, 740, 820, 204, C.panel, { borderColor: '#334155', radius: 18, z: 2 }),
    text(vid(9, 4031), 'discharge-title', 'LINHA DE RECALQUE', 688, 754, 300, 34, { fontSize: 21, weight: 800, z: 8 }),
    line(vid(9, 4032), 'pipe-main', 720, 824, 650, C.pipe, 18, 6),
    text(vid(9, 4033), 'pressure-label', 'PRESSÃO', 730, 846, 130, 24, { fontSize: 14, color: C.muted, weight: 700, z: 8 }),
    valueDisplay(EEE_HMI.elements.mainPressure, 'main-pressure', 'Pressão de recalque', T.dischargePressure, 716, 874, 190, 1),
    text(vid(9, 4035), 'total-flow-label', 'VAZÃO', 990, 846, 130, 24, { fontSize: 14, color: C.muted, weight: 700, z: 8 }),
    valueDisplay(vid(9, 4036), 'main-flow', 'Vazão total', T.totalFlow, 976, 874, 190, 1),
    text(vid(9, 4037), 'duty-label', 'BOMBA DE VEZ', 1250, 846, 150, 24, { fontSize: 14, color: C.muted, weight: 700, z: 8 }),
    valueDisplay(vid(9, 4038), 'main-duty', 'Bomba de vez', T.dutyPump, 1236, 874, 190, 0),
  ...
  return {
    id: EEE_HMI.screens.overview.id,
    key: EEE_HMI.screens.overview.key,
    name: 'EEE Principal',
    route: '/eee',
    elements,
    properties: {},
    context: {},
    metadata: { application: 'eee-demo', role: 'startup' }
  };
}

function kpiPanel(baseId: number, titleValue: string, specs: Array<[string, TagSpec, number]>) {
  const elements: any[] = [
    rect(vid(9, baseId), `panel-${baseId}`, 80, 210, 1760, 720, C.panel, { radius: 18, z: 1 }),
...
function secondaryScreen(id: string, key: string, name: string, elements: any[]) {
  return {
    ...
    key,
    name,
    route: `/eee/${key.split('.').pop()}`,
    elements: [
      rect(vid(9, 7000 + navHash(key)), `canvas-${key}`, 0, 82, 1920, 998, C.canvas, { borderWidth: 0, radius: 0, z: 0 }),
      ...navElements(key),
      ...elements
    ],
    properties: {},
...
function instrumentationScreen() {
  return secondaryScreen(
    EEE_HMI.screens.instrumentation.id,
    EEE_HMI.screens.instrumentation.key,
    'Instrumentação',
    kpiPanel(5000, 'INSTRUMENTAÇÃO E PROCESSO', [
      ['Nível do poço', T.level, 1],
      ['Vazão afluente', T.inflow, 1],
      ['Vazão de recalque', T.totalFlow, 1],
      ['Pressão de recalque', T.dischargePressure, 2],
      ['Pressão P01', T.p01Pressure, 2],
      ['Pressão P02', T.p02Pressure, 2],
      ['Vazão P01', T.p01Flow, 1],
      ['Vazão P02', T.p02Flow, 1]
    ])
  );
}
...
function electricalScreen() {
  return secondaryScreen(
    EEE_HMI.screens.electrical.id,
    EEE_HMI.screens.electrical.key,
    'Sistema Elétrico',
    kpiPanel(5200, 'SISTEMA ELÉTRICO', [
      ['Corrente P01', T.p01Current, 1],
      ['Frequência P01', T.p01Frequency, 1],
      ['Pressão P01', T.p01Pressure, 2],
      ['Corrente P02', T.p02Current, 1],
      ['Frequência P02', T.p02Frequency, 1],
      ['Pressão P02', T.p02Pressure, 2]
    ])
  );
}
...
function operationScreen() {
  const elements: any[] = [
    ...
    text(vid(9, 5402), 'operation-info', 'Comandos seguem EliteSCADA Command → TAG de solicitação → Server Script Active.', 120, 300, 1500, 36, { fontSize: 17, color: C.muted }),
  ...
  return secondaryScreen(EEE_HMI.screens.operation.id, EEE_HMI.screens.operation.key, 'Operação', elements);
}
...
function trendsScreen() {
  ...
  const trend = {
    id: EEE_HMI.elements.trend,
    key: 'eee-process-trend',
    type: 'core.trend',
    properties: {
      x: 100,
      y: 220,
...
function alarmsEventsScreen() {
  return secondaryScreen(EEE_HMI.screens.alarmsEvents.id, EEE_HMI.screens.alarmsEvents.key, 'Alarmes e Eventos', [
    ...
    {
      id: EEE_HMI.elements.alarmBrowser,
      key: 'eee-alarm-browser',
      ...
      properties: {
        ...
        strokeWidth: 1,
        cornerRadius: 12,
        browserConfig: { version: 1, mode: 'current', lifecycle: 'active', area: 'EEE', pageSize: 20 }
      }
    },
    {
      id: EEE_HMI.elements.eventBrowser,
      key: 'eee-event-browser',
      ...
      properties: {
        ...
        strokeWidth: 1,
        cornerRadius: 12,
        browserConfig: { version: 1, source: 'server-script', area: 'EEE', pageSize: 30 }
      }
    }
  ]);
}
...
function pumpPopup(
  ...
  key: string,
  name: string,
  specs: { current: TagSpec; frequency: TagSpec; flow: TagSpec; pressure: TagSpec },
  commands: { start: string; stop: string; fault: string }
) {
  const p01 = key.endsWith('p01');
  return {
    id,
    key,
    name,
    ...
    properties: {},
    context: {},
    metadata: { application: 'eee-demo', role: 'pump-popup' },
    elements: [
      rect(vid(9, p01 ? 6001 : 6101), `${key}-panel`, 0, 0, 880, 600, '#0F1A2C', { borderColor: '#64748B', borderWidth: 2, radius: 18, z: 1 }),
      text(vid(9, p01 ? 6002 : 6102), `${key}-title`, name, 40, 28, 520, 44, { fontSize: 30, weight: 900, z: 5 }),
      text(vid(9, p01 ? 6003 : 6103), `${key}-subtitle`, 'Detalhe operacional · comandos mediados pelo Runtime', 40, 74, 680, 30, { fontSize: 16, color: C.muted, z: 5 }),
      text(vid(9, p01 ? 6010 : 6110), `${key}-current-label`, 'CORRENTE', 60, 150, 160, 28, { fontSize: 14, color: C.muted, weight: 700 }),
      valueDisplay(vid(9, p01 ? 6011 : 6111), `${key}-current`, 'Corrente', specs.current, 50, 184, 220, 1),
      text(vid(9, p01 ? 6012 : 6112), `${key}-freq-label`, 'FREQUÊNCIA', 330, 150, 160, 28, { fontSize: 14, color: C.muted, weight: 700 }),
      valueDisplay(vid(9, p01 ? 6013 : 6113), `${key}-frequency`, 'Frequência', specs.frequency, 320, 184, 220, 1),
      text(vid(9, p01 ? 6014 : 6114), `${key}-pressure-label`, 'PRESSÃO', 600, 150, 160, 28, { fontSize: 14, color: C.muted, weight: 700 }),
      valueDisplay(vid(9, p01 ? 6015 : 6115), `${key}-pressure`, 'Pressão', specs.pressure, 590, 184, 220, 2),
      text(vid(9, p01 ? 6016 : 6116), `${key}-flow-label`, 'VAZÃO', 60, 280, 160, 28, { fontSize: 14, color: C.muted, weight: 700 }),
      valueDisplay(vid(9, p01 ? 6017 : 6117), `${key}-flow`, 'Vazão', specs.flow, 50, 314, 220, 1),
      button(vid(9, p01 ? 6020 : 6120), `${key}-start`, 'PARTIR', 330, 314, 220, { kind: 'executeCommand', targetKey: null, commandId: commands.start, parameters: null }, '#14532D'),
...
export function buildEeeDemoPackage(base: any): any {
  ...
  packageData.popups = [
    pumpPopup(
      EEE_HMI.popups.p01.id,
      EEE_HMI.popups.p01.key,
      'BOMBA P01',
    ...
    pumpPopup(
      EEE_HMI.popups.p02.id,
      EEE_HMI.popups.p02.key,
      'BOMBA P02',
      { current: T.p02Current, frequency: T.p02Frequency, flow: T.p02Flow, pressure: T.p02Pressure },
      { start: EEE_IDS.commands.p02Start, stop: EEE_IDS.commands.p02Stop, fault: EEE_IDS.commands.injectP02Fault }
    )
  ];
  packageData.startupScreenId = EEE_HMI.screens.overview.id;
  return packageData;
}
