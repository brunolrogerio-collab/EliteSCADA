...
static UA_StatusCode addInt32Variable(UA_Server *server) {
    UA_Int32 initialValue = 7;
    UA_VariableAttributes attr = UA_VariableAttributes_default;
    UA_Variant_setScalar(&attr.value, &initialValue, &UA_TYPES[UA_TYPES_INT32]);
    attr.displayName = UA_LOCALIZEDTEXT("en-US", "Secure Counter");
    attr.dataType = UA_TYPES[UA_TYPES_INT32].typeId;
    attr.accessLevel = UA_ACCESSLEVELMASK_READ | UA_ACCESSLEVELMASK_WRITE;
    return UA_Server_addVariableNode(server,
        UA_NODEID_STRING(LAB_NAMESPACE_INDEX, "Lab.SecureCounter"),
        UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
        UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
        UA_QUALIFIEDNAME(LAB_NAMESPACE_INDEX, "Secure Counter"),
...
int main(int argc, char **argv) {
    ...
    if(namespaceIndex != LAB_NAMESPACE_INDEX || addInt32Variable(server) != UA_STATUSCODE_GOOD) {
        UA_Server_delete(server);
        return 6;
    }
