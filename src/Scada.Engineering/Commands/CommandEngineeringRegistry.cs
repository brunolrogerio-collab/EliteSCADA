...
public sealed class InMemoryCommandEngineeringRegistry : ICommandEngineeringRegistry
{
    ...
    public void Upsert(CommandEngineeringDto command)
    {
        ...
        var normalized = command with
        {
            ...
            TagPath = string.IsNullOrWhiteSpace(command.TagPath) ? null : command.TagPath.Trim(),
            Area = string.IsNullOrWhiteSpace(command.Area) ? null : command.Area.Trim(),
            EquipmentPath = string.IsNullOrWhiteSpace(command.EquipmentPath) ? null : command.EquipmentPath.Trim()
        };
